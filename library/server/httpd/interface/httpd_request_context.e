note
	description: "[
			...
			This object is created by {HTTPD_APPLICATION}.new_request_context
		]"
	...
	initialize
		...
		do
			...
			if attached environment_variables.request_uri as rq_uri then
				p := rq_uri.index_of ('?', 1)
				if p > 0 then
					environment_variables.add_variable (rq_uri.substring (1, p-1), {HTTPD_ENVIRONMENT_NAMES}.self)
				else
					environment_variables.add_variable (rq_uri, {HTTPD_ENVIRONMENT_NAMES}.self)
				end
			end
			environment_variables.add_variable (date_time_utilities.unix_time_stamp (Void).out, {HTTPD_ENVIRONMENT_NAMES}.request_time)
		end
...
feature -- Access environment information
	...
	request_time: detachable DATE_TIME
		...
		do
			if
				attached environment_variable ({HTTPD_ENVIRONMENT_NAMES}.request_time) as t and then
				t.is_integer_64
			then
